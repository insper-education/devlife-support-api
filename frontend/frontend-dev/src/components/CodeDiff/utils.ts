...
export function processBlocks(diff: Change[]) {
  ...
  diff.forEach((currentBlock, index) => {
    ...
    const nextIsAddition = !nextBlock || !nextBlock.added;
    const previousIsDeletion = !previousBlock || !previousBlock?.removed;

    const isAdditionOnly = isAddition && !previousIsDeletion;
    const isDeletionOnly = isDeletion && !nextIsAddition;
