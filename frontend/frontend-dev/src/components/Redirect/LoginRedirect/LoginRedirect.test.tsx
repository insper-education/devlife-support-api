...
const setupMockUser = (create: boolean) => {
...
describe("LoginRedirect", () => {
  ...
  it("doesn't redirect to login if user is not found, but is already at the login screen", async () => {
    setupMockUser(false);
  ...
  it("doesn't redirect to login if user is found and is already at the login screen", async () => {
    setupMockUser(true);
    const history = renderRoutes(routes.LOGIN);
    expect(history.location.pathname).toBe(routes.LOGIN);
    expect(screen.queryByText("Target Page")).not.toBeInTheDocument();
  ...
  it("doesn't redirect to login if user is found", () => {
    setupMockUser(true);
    const history = renderRoutes(routes.STUDENT_HOME);
